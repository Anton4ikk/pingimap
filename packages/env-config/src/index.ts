...
function parseEnvNumber(envValue: string | undefined, defaultValue: number): number {
  if (!envValue) {
    return defaultValue;
  }
...
function createConfig(): Config {
  const pingTimeoutMs = clamp(
    parseEnvNumber(process.env.PING_TIMEOUT_MS, DEFAULT_CONFIG.pingTimeoutMs),
    MIN_VALUES.pingTimeoutMs,
    MAX_VALUES.pingTimeoutMs
  );

  const fastThresholdMs = clamp(
    parseEnvNumber(process.env.FAST_THRESHOLD_MS, DEFAULT_CONFIG.fastThresholdMs),
    MIN_VALUES.fastThresholdMs,
    MAX_VALUES.fastThresholdMs
  );

  const slowThresholdMs = clamp(
    parseEnvNumber(process.env.SLOW_THRESHOLD_MS, DEFAULT_CONFIG.slowThresholdMs),
    MIN_VALUES.slowThresholdMs,
    MAX_VALUES.slowThresholdMs
  );
